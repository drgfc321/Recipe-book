...
public class MealPlanView extends VerticalLayout {
    ...
    private HorizontalLayout createHeader() {
        ...
        Button autoGenBtn = new Button(getTranslation("mealPlan.autoGenerate", "Auto-generate"),
                VaadinIcon.MAGIC.create(), e -> openAutoGenerateDialog());
        autoGenBtn.addThemeVariants(ButtonVariant.LUMO_PRIMARY);

        HorizontalLayout header = new HorizontalLayout(title, weekNav, autoGenBtn);
        header.setWidthFull();
    ...
    private Div createFilledCell(LocalDate date, String mealSlot, String slotCss, MealPlanResponse meal) {
        ...
        name.addClickListener(e -> UI.getCurrent().navigate("recipes/" + meal.recipe().id()));
    ...
    private void openAutoGenerateDialog() {
        ...
        H2 dialogTitle = new H2(getTranslation("mealPlan.autoGenerate.title", "Auto-Generate Meal Plan"));
        ...
        Checkbox replaceCheckbox = new Checkbox(
                getTranslation("mealPlan.autoGenerate.replace", "Replace existing meals"));
        replaceCheckbox.setValue(false);

        Checkbox pantryCheckbox = new Checkbox(
                getTranslation("mealPlan.autoGenerate.preferPantry", "Prefer pantry ingredients"));
        pantryCheckbox.setValue(true);

        Span hint = new Span(getTranslation("mealPlan.autoGenerate.hint",
                "Empty slots will be filled with recipes that best match your nutrition targets and available ingredients."));
        hint.getStyle()
        ...
        Button generateBtn = new Button(getTranslation("mealPlan.autoGenerate.run", "Generate"),
        ...
        generateBtn.addClickListener(e -> {
            ...
            try {
                List<MealPlanResponse> generated = mealPlanService.autoGenerateMealPlan(
                        weekStart, replaceCheckbox.getValue(), pantryCheckbox.getValue());
                dialog.close();
                loadWeek();
                if (generated.isEmpty()) {
                    Notification.show(getTranslation("mealPlan.autoGenerate.noRecipes",
                            "No recipes available to generate a meal plan"), 3000, Notification.Position.MIDDLE);
                } else {
                    Notification.show(getTranslation("mealPlan.autoGenerate.success",
                                    "{0} meals generated successfully!", String.valueOf(generated.size())),
                            3000, Notification.Position.BOTTOM_START)
                            .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                }
            } catch (Exception ex) {
                generateBtn.setEnabled(true);
                Notification.show(getTranslation("mealPlan.autoGenerate.error",
                        "Failed to generate meal plan") + ": " + ex.getMessage(),
                        3000, Notification.Position.MIDDLE)
                        .addThemeVariants(NotificationVariant.LUMO_ERROR);
            }
        });

        VerticalLayout content = new VerticalLayout(replaceCheckbox, pantryCheckbox, hint, generateBtn);
